...
function normalizeLedger(raw) {
  ...
  if (!Number.isFinite(Number(ledger.totals.submittedRequestSeconds))) {
    ledger.totals.submittedRequestSeconds = ledger.requests
      .filter((request) => request.status !== 'duplicate-slot' && request.status !== 'budget-rejected' && request.status !== 'rejected_missing_key')
      .reduce((total, request) => total + (Number(request.durationSeconds) || 0), 0);
  }
  if (!Number.isFinite(Number(ledger.totals.acceptedSeconds))) {
...
async function processVideo(videoPath, workDirectory, args, character, sourceHash, requestId, promptHash) {
  ...
  const outputPublicRoot = '/bullshit-factory/motion/v1/' + character.id + '/' + cleanSlug(args.action + '-' + args.emotion);
