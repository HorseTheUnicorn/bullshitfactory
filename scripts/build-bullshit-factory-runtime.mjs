...
if (errors.length) {
  ...
  await fs.writeFile(inventoryPath, `${JSON.stringify({
    ...
    tvOnly: { id: orangeIdiot.id, displayName: orangeIdiot.displayName, sceneId: orangeIdiot.sceneId, standaloneSceneId: orangeIdiot.standaloneSceneId, standaloneBackground: orangeIdiot.standaloneBackground, view: orangeIdiot.view, preview: orangeIdiot.preview, talkingFrames: orangeTalkingFrames, motionRegistry: orangeIdiot.animationContract?.motionRegistry || '/bullshit-factory/production/motion-registry.json', motionPolicy: motionRegistry.runtimePolicy, acceptedMotionClipIds: orangeRequiredMotionClips.filter(Boolean).map((clip) => clip.id), mainCast: false },
