...
export const VOICE_AUDITION_DURATION_BOUNDS = Object.freeze({ min: 12, max: 30, targetMin: 15, targetMax: 25 });
export const DEFAULT_AUDITION_SCRIPT = 'All right, normal shift: the machine hums, the coffee is cold, and the paperwork is lying. What the hell is that? Put it down before I lose my shit! Wait—did it fix itself? It fixed itself! Fantastic, a miracle with a warranty problem. I am not impressed, and I am documenting this carefully because tomorrow somebody will swear this was intentional. Why is the red light blinking? Never mind. Keep your hands off the button. Great. We are doomed.';
...
const VOICE_BLUEPRINTS = Object.freeze({
  ...
  dooby: [
    { name: 'LACONIC', direction: 'relaxed low-mid drawl, loose consonants and dry deadpan timing', recipe: voicePlan([{ voice: 'am_echo', weight: 0.62 }, { voice: 'am_liam', weight: 0.38 }], { speed: 0.9, pitchSemitones: -1.2, formantRatio: 0.979, eq: { lowShelfDb: 0.8, lowMidDb: 0.3, presenceDb: -0.7, highShelfDb: -0.2 }, compression: { ratio: 1.8, thresholdDb: -18 }, effects: { brightness: 0.02, resonance: 0.14, chorus: 0.02 } }) },
    { name: 'SIDEWAYS', direction: 'airy, conversational drawl with a lazy lift on questions', recipe: voicePlan([{ voice: 'am_puck', weight: 0.52 }, { voice: 'af_aoede', weight: 0.48 }], { speed: 0.96, pitchSemitones: 0.3, formantRatio: 1.023, eq: { lowShelfDb: -0.5, presenceDb: 0.4, highShelfDb: 0.7 }, compression: { ratio: 2 }, effects: { brightness: 0.18, nasality: 0.04, chorus: 0.015 } }) },
    { name: 'SLEEPY', direction: 'warm, round and very unhurried with a soft barroom rasp', recipe: voicePlan([{ voice: 'am_michael', weight: 0.56 }, { voice: 'af_nova', weight: 0.44 }], { speed: 0.87, pitchSemitones: -2, formantRatio: 0.97, eq: { lowShelfDb: 1.5, lowMidDb: 0.7, presenceDb: -1, highShelfDb: -0.8 }, compression: { ratio: 2.1 }, effects: { resonance: 0.22, rasp: 0.08, saturation: 0.03, chorus: 0.015 } }) },
  ],
  spaulding: [
    { name: 'DRAWL', direction: 'warm resonant baritone with a weathered Western-style drawl and measured authority', recipe: voicePlan([{ voice: 'am_onyx', weight: 0.62 }, { voice: 'bm_george', weight: 0.38 }], { speed: 0.93, pitchSemitones: -1.8, formantRatio: 0.974, eq: { lowShelfDb: 1.7, lowMidDb: 0.9, presenceDb: -0.3, highShelfDb: -0.9 }, compression: { ratio: 2.6 }, effects: { resonance: 0.24, rasp: 0.2, saturation: 0.06 } }) },
    { name: 'STORYTELLER', direction: 'cleaner, musical and gently drawled like a patient frontier storyteller', recipe: voicePlan([{ voice: 'bm_lewis', weight: 0.58 }, { voice: 'am_adam', weight: 0.42 }], { speed: 0.99, pitchSemitones: -0.6, formantRatio: 1.006, eq: { lowShelfDb: 0.7, lowMidDb: 0.3, presenceDb: 0.7, highShelfDb: 0.4 }, compression: { ratio: 2.3 }, effects: { brightness: 0.1, rasp: 0.13, saturation: 0.03 } }) },
    { name: 'GRAVEL', direction: 'low, slow and lightly gravelled once the punchline lands', recipe: voicePlan([{ voice: 'am_fenrir', weight: 0.54 }, { voice: 'bm_fable', weight: 0.46 }], { speed: 0.95, pitchSemitones: -1, formantRatio: 0.982, eq: { lowShelfDb: 1.1, lowMidDb: 0.7, presenceDb: 0.5, highShelfDb: -0.5 }, compression: { ratio: 3 }, effects: { rasp: 0.24, saturation: 0.07, resonance: 0.15 } }) },
  ],
  string: [
...
export function voiceFilterForProfile(profile = {}, { useRubberband = false, normalize = true } = {}) {
  ...
  // FFmpeg's `acompressor` makeup option is a linear gain (1-64), while the
  // stored profile is intentionally expressed in dB for human-readable
  // metadata. Convert it at the filter boundary and keep negative/zero dB at
  // unity so the command remains valid on every supported FFmpeg build.
  const makeupGain = clamp(10 ** (compression.makeupDb / 20), 1, 64);
  filters.push(`acompressor=threshold=${filterNumber(compression.thresholdDb)}dB:ratio=${filterNumber(compression.ratio)}:attack=${filterNumber(compression.attackMs)}:release=${filterNumber(compression.releaseMs)}:makeup=${filterNumber(makeupGain)}`);
  if (normalize) {
    filters.push('loudnorm=I=-18:LRA=7:TP=-2:linear=true');
    // FFmpeg's limiter auto-level is enabled by default. That make-up gain
    // can raise an otherwise safe normalized take back to 0 dB, so disable it
    // and keep the explicit 0.95 peak ceiling in charge.
    filters.push('alimiter=limit=0.95:level=0');
  }
  return filters.join(',');
}
