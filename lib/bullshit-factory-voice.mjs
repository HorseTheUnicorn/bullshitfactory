...
export function normalizeVoiceRecipe(value = {}) {
  ...
  return {
    ...
    embedding: raw.embedding && typeof raw.embedding === 'object'
      ? {
        kind: String(raw.embedding.kind || 'kokoro-vector-recipe').slice(0, 80),
        id: String(raw.embedding.id || '').slice(0, 120) || null,
        source: String(raw.embedding.source || 'Kokoro stock voice vectors').slice(0, 240),
        reusable: raw.embedding.reusable !== false,
      }
