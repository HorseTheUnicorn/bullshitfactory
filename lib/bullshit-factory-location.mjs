...
// time, so transparent authoring padding cannot make a character float.
...
// Bork's authored motion canvas is larger than the human sprites so his ears
// and tail have room to move. Keep that authoring canvas, but render the dog at
// a believable cast-relative size and preserve the same feet/ground anchor.
export const CHARACTER_SCALE_MULTIPLIERS = Object.freeze({ bork: 0.72 });
...
export const LOCATION_SPECS = Object.freeze({
  ...
  [ORANGE_IDIOT_STANDALONE_SCENE_ID]: sceneSpec(
    ...
    },
    {
      ...
      broadcastAnchors: {
        orangeIdiot: {
          // Orange Idiot remains a locked south-facing performance. The
          // renderer may translate the sprite during the broadcast, but it
          // must not rotate him into a profile while he is speaking.
          centerX: 192,
          leftX: 98,
          rightX: 286,
          spriteWidth: 64,
          ...
          talkingMotion: 'short-burst talk with intentional pauses and emphasis changes',
          pacingDirection: 'left-to-right while speaking, then return to center',
          pacingPolicy: 'activate lateral travel only while measured Orange Idiot speech is audible',
          pacing: {
            initialHoldMs: 1000,
            travelToLeftMs: 1400,
            leftHoldMs: 900,
          ...
          // The podium masks the figure baked into the supplied house
          // background and gives the broadcast a stable center return point.
          podium: {
            left: 158,
            top: 143,
            width: 68,
...
export function resolveScenePlacement({ sceneId = 'factory-floor', characterId = 'rookboss', walkBand = 'middle', x = 0.5, near = null, frameGeometry = null } = {}) {
  ...
  const characterScale = Number(CHARACTER_SCALE_MULTIPLIERS[characterId]) || 1;
  const scale = resolvedBand.scale * characterScale;
  ...
  return {
    ...
    // Keep the measured H3 envelope attached to the placement so a later
    // crowd-rebalance into another walk band resolves the same silhouette.
    // Without this, band reassignment silently fell back to canonical art
    // geometry and the renderer could re-expand the sprite into a collision.
    layoutGeometry: frameGeometry || null,
  };
}
...
function reResolvePlacementBand(placement, sceneId, bandId) {
  const base = resolveScenePlacement({
    ...
    frameGeometry: placement.layoutGeometry || null,
  });
  return {
    ...base,
    intent: {
...
function separateVisiblePlacements(placements, scene) {
  ...
  for (const placement of ordered) {
    const current = resolved.get(placement.characterId) || placement;
    const band = bandById(scene, current.walkBand);
    const leftExtent = placementLeftExtent(current);
    const rightExtent = placementRightExtent(current);
    const minimum = band.xMin + leftExtent;
    const maximum = band.xMax - rightExtent;
    let feetX = Math.max(minimum, Math.min(maximum, current.feet.x));
    for (const previous of swept) {
      if (!verticalPlacementOverlap(current, previous)) continue;
      const previousRight = previous.visibleBounds?.right ?? (previous.sprite.left + previous.sprite.width - 1);
      feetX = Math.max(feetX, previousRight + MIN_VISIBLE_ACTOR_GAP_PX + 1 + leftExtent);
    }
    feetX = Math.min(feetX, maximum);
    const packed = repositionPlacementX(current, feetX, scene);
    swept.push(packed);
    resolved.set(placement.characterId, packed);
  }
