...
const DISALLOWED_RUNTIME_CUE_IDS = new Set([
  'bf-factory-ambience',
  'bf-theme-main',
  'bf-intro-01',
  'bf-outro-01',
  ...
  'bf-dusty-psych-rock',
  'bf-bit-crushed-rock',
  'bf-dockside-shanty-rock',
  'bf-rust-belt-blues',
  'bf-late-night-rock',
  'bf-warehouse-pulse',
  'bf-corporate-jingle',
  'bf-cartoon-sting',
  'bf-cartoon-drum-break',
]);
...
function runtimeCueAllowed(cue) {
  const assetId = text(cue?.assetId).toLowerCase();
  if (DISALLOWED_RUNTIME_CUE_IDS.has(assetId)) return false;
  const kind = text(cue?.kind).toLowerCase();
  if (!['music', 'ambience', 'stinger'].includes(kind)) return true;
  return ALLOWED_MUSIC_CUE_IDS.has(assetId);
}
...
function findAssetForCue(cue, assets) {
  ...
  // Never replace a missing permitted music cue with another music track.
  // That would silently reintroduce a bed or stinger under the theme/String
  // only runtime policy. Optional music therefore falls back to silence.
  if (['music', 'ambience', 'stinger'].includes(text(cue.kind).toLowerCase())) return null;
