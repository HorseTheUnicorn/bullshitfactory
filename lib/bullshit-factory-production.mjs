...
import { choreographPerformance } from './bullshit-factory-performance.mjs';
...
// Keep a small authored reaction/final-button tail without leaving a long-form
// segment silent after its last line. Measured speech is distributed up to this
// boundary after Kokoro has produced the real takes.
export const VOICE_REACTION_TAIL_MS = 1500;
export const MAX_DISTRIBUTED_VOICE_GAP_MS = 7000;
export const MAX_DIALOGUE_TAIL_MS = 6000;
export const SHARED_TTS_SPEED = 1.05;
// Production measurements of the selected cast profiles average close to 140
// WPM. This is a planning calibration for word budgets and draft timing; it
// does not change the Kokoro playback speed.
export const SPEECH_CALIBRATED_WPM = 140;
export const SPEECH_START_RESERVE_MS = 900;
// The operator requested a literal two-millisecond handoff between takes. Keep
// this separate from speech speed and from the authored reaction tail.
export const SPEAKER_HANDOFF_GAP_MS = 2;
...
export const ORANGE_IDIOT_SCENE_ID = ORANGE_IDIOT_YARD_SCENE_ID;
export const ORANGE_IDIOT_STANDALONE_SCENE_ID = ORANGE_IDIOT_YARD_SCENE_ID;
export const ORANGE_IDIOT_VOICE_PROFILE = Object.freeze({
  provider: 'kokoro-loopback',
  voiceId: 'bm_daniel',
  language: 'en-us',
  accent: 'original New York/Queens-inspired character delivery; not an imitation of a real person',
  pitch: 'low-to-mid',
  timbre: 'slightly nasal, raspy, breathy, and mildly congested',
  delivery: 'short bursts with frequent pauses, repetitions, stretched vowels, and abrupt emphasis changes',
  pitchMultiplier: 0.82,
  speed: 1.0,
});

export function normalizeOrangeIdiotSpeechDurationSeconds(value, durationSeconds = 30) {
  const normalizedDuration = Math.max(10, Number(durationSeconds) || 30);
  const maximum = Math.max(ORANGE_IDIOT_SPEECH_MIN_SECONDS, normalizedDuration - SCRIPT_END_BUFFER_MS / 1000);
  const requested = Number(value);
  if (!Number.isFinite(requested) || requested <= 0) return 0;
  return Number(Math.min(maximum, Math.max(ORANGE_IDIOT_SPEECH_MIN_SECONDS, requested)).toFixed(3));
}
...
export function orangeIdiotSpeechTargetSeconds(value, durationSeconds = 30, standalone = false) {
  ...
  return normalizeOrangeIdiotSpeechDurationSeconds(Number(durationSeconds) || 30, durationSeconds);
}
...
function normalizeVoiceTimelineEvents(events) {
...
export function serializeVoiceTimeline(events, durationSeconds = 30, gapMs = SPEAKER_HANDOFF_GAP_MS, tailMs = SCRIPT_END_BUFFER_MS) {
  ...
  const normalized = normalizeVoiceTimelineEvents(events);
  const timeline = [];
  let cursor = 0;
...
/**
 * Spread measured vocal takes through the requested runtime. This preserves
 * event order and the first authored timestamp, but uses the real take
 * durations to allocate natural gaps. The gap is capped so an under-written
 * script is not disguised as a forty-second pause; the caller can reject that
 * case through its coverage validation.
 */
export function spreadVoiceTimeline(events, durationSeconds = 30, gapMs = SPEAKER_HANDOFF_GAP_MS, tailMs = VOICE_REACTION_TAIL_MS) {
  const gap = Math.max(0, Math.min(MAX_DISTRIBUTED_VOICE_GAP_MS, Math.round(Number(gapMs) || SPEAKER_HANDOFF_GAP_MS)));
  const normalized = normalizeVoiceTimelineEvents(events);
  if (normalized.length <= 1) return serializeVoiceTimeline(events, durationSeconds, gap, tailMs);

  const firstStartMs = normalized[0].requestedStartMs;
  const totalDurationMs = normalized.reduce((total, item) => total + item.durationMs, 0);
  const requestedSeconds = Number(durationSeconds);
  const safeDurationSeconds = Number.isFinite(requestedSeconds) && requestedSeconds > 0 ? requestedSeconds : 30;
  const safeTailMs = Math.max(SCRIPT_END_BUFFER_MS, Math.round(Number(tailMs) || VOICE_REACTION_TAIL_MS));
  const desiredEndMs = Math.max(firstStartMs + totalDurationMs, Math.round(safeDurationSeconds * 1000) - safeTailMs);
  const desiredGapMs = (desiredEndMs - firstStartMs - totalDurationMs) / (normalized.length - 1);
  const distributedGap = Math.max(gap, Math.min(MAX_DISTRIBUTED_VOICE_GAP_MS, Math.round(desiredGapMs)));
  const timeline = [];
  let cursor = firstStartMs;
  for (const [index, item] of normalized.entries()) {
    const startMs = index ? cursor + distributedGap : firstStartMs;
    const endMs = startMs + item.durationMs;
    timeline.push({ ...item.event, startMs, endMs, durationMs: item.durationMs, wasTrimmed: false });
    cursor = endMs;
  }
  return timeline;
}
...
export const FACTORY_PROPS = Object.freeze([
  ...
  },
  {
    ...
    anchors: { 'break-room': 'table', 'employee-bar': 'bar_center', 'factory-floor': 'workbench' },
  },
  {
    id: 'crt-keyboard',
    label: 'CRT KEYBOARD',
  ...
  },
  {
    ...
    anchors: { 'factory-floor': 'server_rack', 'server-room': 'left_rack', 'break-room': 'table' },
  },
]);
...
export function buildMotionPlan(castIds, dialogue, barkEvents, seed = 1, fps = 12, stageDirections = [], durationSeconds = 30, options = {}) {
  ...
  const actors = actorIds.map((actorId, index) => ({
    ...
    idle: { loop: true, fps, framePolicy: 'approved-local-motion-registry' },
  }));
  ...
  const choreography = choreographPerformance({
    actorIds,
    ...
    durationMs,
    seed,
    pacingProfile: options?.pacingProfile || null,
    tailMs: SCRIPT_END_BUFFER_MS,
    timingSource: options?.timingSource || 'draft-estimate',
  });
  return {
    fps,
    rootPinned: true,
    ...
    actors,
    cues: choreography.events,
    states: choreography.states.length ? choreography.states : states,
    shots: choreography.camera.length ? choreography.camera : buildMotionShots(actorIds, dialogueLines, barkEvents, durationMs, props),
    performanceTimeline: choreography,
    pacingProfile: choreography.pacingProfile,
    qualityMetrics: choreography.metrics,
    semanticDirections,
    collisionPolicy: {
      ...
      choreography: 'one-base-state-plus-one-compatible-overlay',
    },
  };
}
...
export function buildOrangeIdiotTvPlan(text, sceneId, durationSeconds = 30, source = 'operator-supplied-speech-excerpt', position = 'ending', speechDurationSeconds = 0) {
  ...
  if (!normalizedText || sceneId !== ORANGE_IDIOT_STANDALONE_SCENE_ID) return [];
  ...
  return [{
    ...
    performance: {
      headTarget: 'camera',
      gaze: 'south-facing direct-to-camera',
      pacing: 'left-to-right while speaking, then return to center',
      pacingPolicy: 'deterministic travel is active only during the measured speech window',
      delivery: ORANGE_IDIOT_VOICE_PROFILE.delivery,
    },
  }];
}
...
export function buildSegmentDraft({ templateId, seed = 1, durationSeconds = 30, castIds = CAST_IDS, sceneId = null, orangeIdiotSpeechText = '', orangeIdiotRequested = false, orangeIdiotOnly = false, orangeIdiotPosition = 'ending', orangeIdiotSpeechDurationSeconds = 0, orangeIdiotResearch = null, director = 'deterministic-goblin-fallback' } = {}) {
  const template = findTemplate(templateId);
  const effectiveOrangeIdiotOnly = Boolean(orangeIdiotOnly || orangeIdiotRequested);
  const scene = effectiveOrangeIdiotOnly ? findScene(ORANGE_IDIOT_STANDALONE_SCENE_ID) : findScene(sceneId || template.sceneId);
  ...
  const effectiveOrangePosition = effectiveOrangeIdiotOnly
    ? 'full-broadcast'
  ...
  const humans = effectiveOrangeIdiotOnly ? [] : humanPool.slice(0, maxHumanCast);
  const finalCast = effectiveOrangeIdiotOnly ? [] : [...new Set([...humans.slice(0, 9), 'bork'])];
  ...
  const dialogue = effectiveOrangeIdiotOnly ? [] : buildDialogue(template.id, seed, finalCast, normalizedDuration);
  const barkEvents = effectiveOrangeIdiotOnly ? [] : buildBarkEvents(seed, durationSeconds);
  ...
  const motion = buildMotionPlan(finalCast, dialogue, barkEvents, seed, 12, [], normalizedDuration, { props, storyBeats: DEFAULT_STORY_BEATS, timingSource: 'draft-estimate' });
  ...
  return {
    ...
    castIds: finalCast,
    orangeIdiotOnly: effectiveOrangeIdiotOnly,
    ...
    music: { mode: 'none', trackId: null, required: false, status: 'disabled', file: null, provider: 'none', policy: 'opening-theme-and-string-guitar-only' },
...
export function validateSegmentContract(segment, { requireMedia = false, musicTracks = MUSIC_RIGHTS, knownCastIds = CAST_IDS } = {}) {
  ...
  if ((orangeIdiotOnly || tvInterruptions.length) && !isOrangeStandaloneScene) errors.push('Orange Idiot segments must use the standalone Orange Idiot house');
  if (isOrangeStandaloneScene && tvInterruptions.some((event) => event?.mode !== 'house-broadcast')) errors.push('Orange Idiot house broadcasts must use the house broadcast mode');
  ...
  if (!orangeIdiotOnly && semanticMotionV2) {
    const performanceTimeline = segment?.motion?.performanceTimeline;
    const expectedDurationMs = Math.round(Number(segment.durationSeconds) * 1000);
    if (!performanceTimeline || performanceTimeline.schemaVersion !== '1.0') {
      errors.push('semantic animation must include a versioned performance timeline');
    } else {
      if (Number(performanceTimeline.durationMs) !== expectedDurationMs) errors.push('performance timeline duration must match the segment duration');
      if (!['draft-estimate', 'measured-kokoro-audio'].includes(performanceTimeline.timingSource)) errors.push('performance timeline timingSource must be draft-estimate or measured-kokoro-audio');
      if (requireMedia && performanceTimeline.timingSource !== 'measured-kokoro-audio') errors.push('media validation requires measured Kokoro timing');
      if (performanceTimeline.baseStatePolicy !== 'exactly-one-base-state-plus-at-most-one-compatible-overlay') errors.push('performance timeline must declare the one-base-state/one-overlay policy');
      if (JSON.stringify(performanceTimeline.priorityOrder) !== JSON.stringify(['traveling', 'speaking', 'reacting', 'listening', 'idle'])) errors.push('performance timeline priority order is invalid');
      if (!performanceTimeline.metrics || !Number.isFinite(Number(performanceTimeline.metrics.idleListenPercentage))) errors.push('performance timeline must include deterministic quality metrics');
      const timelineEvents = Array.isArray(performanceTimeline.events) ? performanceTimeline.events : [];
      if (timelineEvents.length !== motionCues.length) errors.push('motion cues must be the exact executable performance timeline events');
      if (timelineEvents.some((event) => Number(event.startMs) < 0 || Number(event.endMs) <= Number(event.startMs) || Number(event.endMs) > expectedDurationMs)) errors.push('performance timeline events must stay inside the segment');
      const overlaysByActor = new Map();
      for (const event of timelineEvents.filter((candidate) => candidate.phase === 'overlay')) {
        const overlays = overlaysByActor.get(event.actorId) || [];
        if (overlays.some((prior) => Number(event.startMs) < prior.endMs && Number(event.endMs) > prior.startMs)) errors.push('an actor may not have overlapping performance overlays');
        overlays.push({ startMs: Number(event.startMs), endMs: Number(event.endMs) });
        overlaysByActor.set(event.actorId, overlays);
      }
      const performanceActors = new Set((performanceTimeline.performances || []).map((performance) => performance.actorId));
      if (castIds.some((id) => !performanceActors.has(id))) errors.push('every cast member needs a performance timeline entry');
    }
  ...
  const segmentMusicMode = String(segment?.music?.mode || 'none').trim().toLowerCase();
  const music = musicTracks.find((track) => track.id === segment?.music?.trackId);
  if (segmentMusicMode !== 'none' && (!music || music.status !== 'approved')) errors.push('music track is not approved');
  if (segmentMusicMode === 'none' && segment?.music?.usedInMix === true) errors.push('content music must not be mixed when segment music mode is none');
  if (requireMedia) {
